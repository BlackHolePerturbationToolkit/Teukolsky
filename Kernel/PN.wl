...
If[p==ExpOrder,Identity(*Put[MST,"MSTs"<>ToString[s]<>"genlmO"<>ToString[ExpOrder]<>".wl"]*)];
Print["done ",p],
...
MSTCoefficientsInternalFreq[\[ScriptS]_Integer,\[ScriptL]Var_Symbol,\[ScriptM]Var_,aKerr_,order\[CurlyEpsilon]_Integer,OptionsPattern[]]/;order\[CurlyEpsilon]>13:=Module[{aux,ret,repls,keys,values},
...
values=aux//Values//FixContext;
values=values//ChangeSeriesParameter[#,var]&//Series[#,{var,0,order-1+If[\[ScriptS]>0,2,0]}]&;
values=values+SeriesData[var, 0, {}, order+If[\[ScriptS]>0,2,0], order+If[\[ScriptS]>0,2,0], 1];
...
SeriesMap[function_,expr_,levelspec_:Infinity]:=expr/.a_SeriesData:>SeriesMap[function,a,levelspec];
...
SeriesMapAt[function_,expr_SeriesData,pos_]:=Module[{aux,coeffs},
coeffs=expr[[3]];
aux=MapAt[function,coeffs,pos];
expr//ReplacePart[#,3->aux]&
]
SeriesMapAt[function_,expr_,pos_]:=expr/.a_SeriesData:>SeriesMapAt[function,a,pos];
(*SeriesMap[function_,expr_,levelspec_:Infinity]/;MatchQ[expr,a__ b_SeriesData]:=Module[{aux,coeffs,factor,series},
{factor,series}=expr/.a__ b_SeriesData:>{Times[a],b};
coeffs=series[[3]];
aux=SeriesMap[function,series,levelspec];
...
InactiveSeriesPrefactor[series_SeriesData]:=Module[{aux,factor},
factor=series//SeriesTake[#,1]&//Normal;
aux=series/factor;
aux=Inactive[Times][factor,aux];
aux
]
...
(*Not public*)
ReplacePolyGamma=((#//ReplaceAll[PolyGamma[aa__](*/;(IntegerQ[aa]&&IntegerQ[bb])*):>FunctionExpand[PolyGamma[aa],Assumptions->$Assumptions]])&);


(*Reflections=(#/.\[CapitalGamma][xx_]/;!FreeQ[xx,\[Nu]MST]:>\[Pi]/(Sin[\[Pi] xx]\[CapitalGamma][1-xx])/.PH[xx_,n_]/;!FreeQ[xx,\[Nu]MST]:>(-1)^nPH[-xx-n+1,n])&;*)
...
Options[TeukolskyPointParticleSourceRepls]={"Form"->"Default","InactiveHarmonics"->False};
...
BAmplitudeFreq["Inc",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{aux,coeff,repls,GenSeriesStuff,DoABunchOfStuff,\[CurlyEpsilon],aux1,\[Kappa],\[CurlyEpsilon]p,\[Tau],\[ScriptCapitalK]1,\[ScriptCapitalK]2,\[ScriptCapitalK]2coeff,A,norm,normOp},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[Kappa]=Sqrt[1-a^2];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
...
GenSeriesStuff=If[!NumericQ[\[ScriptL]],((#//Expand//SeriesPlusSimplify[#,{\[ScriptL]>=If[NumericQ[\[ScriptS]],Abs[\[ScriptS]],0]}]&)&),Identity];
...
\[ScriptCapitalK]2coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=Abs[\[ScriptS]]>=0},-((I E^(-I \[Pi] \[Nu]MST) Sin[\[Pi] (\[Nu]MST-\[ScriptS]+I \[CurlyEpsilon])])/Sin[\[Pi] (\[Nu]MST+\[ScriptS]-I \[CurlyEpsilon])])/.repls//DoABunchOfStuff];
...
aux1=If[!NumericQ[\[ScriptL]]||!NumericQ[\[ScriptS]],SeriesData[\[Gamma], 0, {1}, 0, order\[CurlyEpsilon], 1],1];
aux= norm A coeff GenSeriesStuff[aux1 + \[ScriptCapitalK]2coeff \[ScriptCapitalK]2]//DoABunchOfStuff;
aux
]
...
BAmplitudeFreq["Ref",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{aux,coeff,repls,DoABunchOfStuff,GenSeriesStuff,\[CurlyEpsilon],aux1,\[Kappa],\[CurlyEpsilon]p,\[Tau],\[ScriptCapitalK]1,\[ScriptCapitalK]2,\[ScriptCapitalK]2coeff,A,norm,normOp},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[Kappa]=Sqrt[1-a^2];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
...
A=AAmplitudeFreq["-"][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon]]//DoABunchOfStuff//SeriesCollect[#,Log[__]]&;
aux1=If[!NumericQ[\[ScriptL]]||!NumericQ[\[ScriptS]],SeriesData[\[Gamma], 0, {1}, 0, order\[CurlyEpsilon], 1],1];
aux=A norm coeff GenSeriesStuff[aux1 + \[ScriptCapitalK]2coeff \[ScriptCapitalK]2]//DoABunchOfStuff;
aux
]
...
(*If[OptionValue["Simplify"],aux=aux//SeriesCollect[#,{E^__,Log[__],Gamma[__],PolyGamma[__]},(Simplify[#,{1>a>=0}]&)]&];*)
If[OptionValue["Simplify"],aux=aux//InactiveSeriesPrefactor//SeriesExpand[#,E^__]&//ExpandLog[#,\[Omega]>0]&//SeriesCollect[#,{E^__,Log[__],Gamma[__],PolyGamma[__]},(Simplify[#,{1>a>=0}]&)]&//Activate[#,Times]&];
aux
]
...
CAmplitudeFreq["Inc",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{aux,aux2,coeff,ReflectGammas,repls,DoABunchOfStuff,GenSeriesStuff,bracket,\[CurlyEpsilon],\[Kappa],\[CurlyEpsilon]p,\[Tau],K1,K1coeff,K2coeff,K2,A,norm,nMin,nMax,normOp,GammaStuff},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[Kappa]=Sqrt[1-a^2];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
...
normOp=OptionValue["Normalization"];
If[!MemberQ[PossibleNormalizations,normOp],Message[TeukolskyRadialFunctionPN::norm,OptionValue["Normalization"]];normOp="Default"];
DoABunchOfStuff=(#//IgnoreExpansionParameter//SeriesTake[#,order\[CurlyEpsilon]]&//GammaStuff//GenSeriesStuff//ExpandTrig)&;
ReflectGammas=#/.{Gamma[x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>a>0,1>\[Kappa]>0,\[ScriptM]>0}]]==-1):>(\[Pi] Csc[\[Pi] x])/Gamma[1-x],PolyGamma[n_,x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>\[Kappa]>0,1>a>0,\[ScriptM]>0}]]==-1):>-\[Pi]^(n+1) Derivative[n][Cot][\[Pi] x]+(-1)^n PolyGamma[n,1-x]}&;
GenSeriesStuff=If[!NumericQ[\[ScriptL]],((#//Expand//SeriesPlusSimplify[#,{\[ScriptL]>=If[NumericQ[\[ScriptS]],Abs[\[ScriptS]],0]}]&)&),Identity];
GammaStuff=(#//ExpandPolyGamma[#,\[ScriptL]]&//ExpandGamma[#,\[ScriptL]]&//ReflectGammas//ExpandPolyGamma[#,\[ScriptL]+1]&//ExpandGamma[#,\[ScriptL]+1]&)&;
If[!NumericQ[\[ScriptL]],GammaStuff=((#//ReflectGammas//ExpandGamma//ExpandPolyGamma)&)];
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,Max[order\[CurlyEpsilon]+2,2]];
(*coeff=-\[ImaginaryI]^(\[ScriptS]-\[Nu]MST) \[ExponentialE]^(-(1/2) \[ImaginaryI] (2 \[Pi] (\[ScriptS]-\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST)+\[Kappa] (\[CurlyEpsilon]+\[Tau]))) \[Pi] \[Kappa]^(-((\[ImaginaryI] \[Kappa] (\[CurlyEpsilon]+\[Tau]))/(1+\[Kappa]))) Csc[2 \[Pi] \[Nu]MST]^2 Csc[\[Pi] (\[ScriptS]+\[ImaginaryI] (\[CurlyEpsilon]+\[Tau]))] Gamma[-\[ScriptS]-\[ImaginaryI] (\[ImaginaryI]+\[CurlyEpsilon]+\[Tau])] /Gamma[1+\[ScriptS]+\[ImaginaryI] \[CurlyEpsilon]+\[ImaginaryI] \[Tau]]Sin[\[Pi] (-\[ScriptS]+\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (\[ScriptS]+\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (\[Nu]MST+\[ImaginaryI] \[Tau])];*)
coeff=-I^(\[ScriptS]-\[Nu]MST) E^(-(1/2) I (2 \[Pi] (\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST)+\[Kappa] (\[CurlyEpsilon]+\[Tau]))) \[Pi] \[Kappa]^(-((I \[Kappa] (\[CurlyEpsilon]+\[Tau]))/(1+\[Kappa]))) Csc[2 \[Pi] \[Nu]MST]^2 (((\[ScriptS]+I (\[CurlyEpsilon]+\[Tau])) Gamma[-\[ScriptS]-I (\[CurlyEpsilon]+\[Tau])]^2)/\[Pi]) Sin[\[Pi] (-\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (\[Nu]MST+I \[Tau])];
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//DoABunchOfStuff];
K1coeff=-((Gamma[1+n+\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST+I \[Tau]] )/(\[Pi] Gamma[1+n-\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST-I \[Tau]] ));
K1coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0,\[Gamma]>0},( \!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(nMax\)]\(K1coeff\  aMST[n]\)\))/.repls//DoABunchOfStuff];
K2coeff=I^(\[Nu]MST+1+\[ScriptS])/I^(-\[Nu]MST+\[ScriptS]) I E^(I \[Pi] \[Nu]MST) Csc[\[Pi] (-\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST)];
K2coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0,\[Gamma]>0},(K2coeff)/.repls//DoABunchOfStuff];
...
bracket=DoABunchOfStuff[(1/K1+K2coeff/(K2))];
coeff=DoABunchOfStuff[norm coeff K1coeff];
aux=(coeff bracket);
If[!NumericQ[\[ScriptL]]||!NumericQ[\[ScriptS]],aux=aux//ReflectGammas];
aux
]


(*CAmplitudeFreq["Inc",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{aux,aux2,coeff,ReflectGammas,repls,DoABunchOfStuff,GenSeriesStuff,\[CurlyEpsilon],\[Kappa],\[CurlyEpsilon]p,\[Tau],K1,K1coeff,K2coeff,K2,A,norm,nMin,nMax,normOp,GammaStuff},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[Kappa]=Sqrt[1-a^2];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
...
If[!NumericQ[\[ScriptL]],GammaStuff=((#//ReflectGammas//ExpandGamma//ExpandPolyGamma)&)];
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,Max[order\[CurlyEpsilon]+2,2]];
coeff=(I^(-\[Nu]MST+\[ScriptS]))(\[ImaginaryI] Csc[2 \[Pi] \[Nu]MST] Sin[\[Pi] (-\[ScriptS]-\[ImaginaryI] \[CurlyEpsilon]-\[Nu]MST)] Sin[\[Pi] (-\[ScriptS]+\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (-\[Nu]MST-\[ImaginaryI] \[Tau])])(I \[Pi] Csc[2 \[Pi] \[Nu]MST] Csc[\[Pi] (\[ScriptS]+I (\[CurlyEpsilon]+\[Tau]))] Gamma[1-\[ScriptS]-I \[CurlyEpsilon]-I \[Tau]])/(E^(1/2 I (2 \[Pi] (\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST)+\[Kappa] (\[CurlyEpsilon]+\[Tau]))) \[Kappa]^((I \[Kappa] (\[CurlyEpsilon]+\[Tau]))/(1+\[Kappa])));
EchoTiming[coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//DoABunchOfStuff],"coeff"];
K1coeff=-((Gamma[1+n+\[ScriptS]+\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST+\[ImaginaryI] \[Tau]] )/(\[Pi] Gamma[1+n-\[ScriptS]-\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST-\[ImaginaryI] \[Tau]] Gamma[1+\[ScriptS]+\[ImaginaryI] \[CurlyEpsilon]+\[ImaginaryI] \[Tau]]));
EchoTiming[K1coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0,\[Gamma]>0},( \!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(nMax\)]\(K1coeff\ aMST[n]\)\))/.repls//DoABunchOfStuff],"K1coeff"];
K2coeff=(E^(I \[Pi] \[Nu]MST) Csc[2 \[Pi] (n+\[Nu]MST)] Gamma[1+n+\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST+I \[Tau]] Sin[\[Pi] (\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (n+\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (n+\[Nu]MST+I \[Tau])])/(\[Pi] Gamma[1+n-\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST-I \[Tau]] Gamma[1+\[ScriptS]+I \[CurlyEpsilon]+I \[Tau]]);
EchoTiming[K2coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0,\[Gamma]>0},(I^(\[Nu]MST+1+\[ScriptS])/I^(-\[Nu]MST+\[ScriptS]) \[ImaginaryI] \[ExponentialE]^(\[ImaginaryI] \[Pi] \[Nu]MST) Csc[\[Pi] (-\[ScriptS]+\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (\[ScriptS]-\[ImaginaryI] \[CurlyEpsilon]+\[Nu]MST)])/.repls//DoABunchOfStuff],"K2coeff"];
EchoTiming[K1=\[ScriptCapitalK]AmplitudeFreq["\[Nu]"][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon]]//DoABunchOfStuff,"K1"];
EchoTiming[K2=\[ScriptCapitalK]AmplitudeFreq["-\[Nu]-1"][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon]]//DoABunchOfStuff//DoABunchOfStuff,"K2"];
AA1=K1coeff//ChangeContext[#,"Teukolsky`PN`Private`"]&;
AA2=K2coeff//ChangeContext[#,"Teukolsky`PN`Private`"]&;
norm=Switch[normOp,
	"Default",(I^(-1-2\[ScriptS]) E^(\[Pi] \[CurlyEpsilon]) E^(I \[Pi]/2 (\[Nu]MST+1+\[ScriptS])))/.repls,
	"SasakiTagoshi",SeriesData[\[Gamma], 0, {1}, 0, order\[CurlyEpsilon], 1],
	"TidalResponse",SeriesData[\[Gamma], 0, {1}, 0, order\[CurlyEpsilon], 1],
...
EchoTiming[aux=(norm coeff K1coeff DoABunchOfStuff[(1/K1+K2coeff/(K2))])//DoABunchOfStuff//DoABunchOfStuff//ReflectGammas,"Assemble"];
aux
]*)
...
EchoTiming[coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//DoABunchOfStuff],"coeff"];
K1coeff=-((I Csc[2 \[Pi] (n+\[Nu]MST)] Gamma[1+n+\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST+I \[Tau]] Sin[\[Pi] (-n-\[ScriptS]-I \[CurlyEpsilon]-\[Nu]MST)] Sin[\[Pi] (-\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (-n-\[Nu]MST-I \[Tau])])/(\[Pi] Gamma[1+n-\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST-I \[Tau]] Gamma[1+\[ScriptS]+I \[CurlyEpsilon]+I \[Tau]]));
EchoTiming[K1coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0,\[Gamma]>0},(I^(-\[Nu]MST+\[ScriptS]) \!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(nMax\)]\(K1coeff\ aMST[n]\)\))/.repls//DoABunchOfStuff],"K1coeff"];
K2coeff=(E^(I \[Pi] \[Nu]MST) Csc[2 \[Pi] (n+\[Nu]MST)] Gamma[1+n+\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST+I \[Tau]] Sin[\[Pi] (\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (n+\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST)] Sin[\[Pi] (n+\[Nu]MST+I \[Tau])])/(\[Pi] Gamma[1+n-\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+n+\[Nu]MST-I \[Tau]] Gamma[1+\[ScriptS]+I \[CurlyEpsilon]+I \[Tau]]);
EchoTiming[K2coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0,\[Gamma]>0},(I^(\[Nu]MST+1+\[ScriptS]) \!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(nMax\)]\(K2coeff\ aMST[n]\)\))/.repls//DoABunchOfStuff],"K2coeff"];
EchoTiming[K1=\[ScriptCapitalK]AmplitudeFreq["\[Nu]"][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon]]//DoABunchOfStuff,"K1"];
EchoTiming[K2=\[ScriptCapitalK]AmplitudeFreq["-\[Nu]-1"][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon]]//DoABunchOfStuff//DoABunchOfStuff,"K2"];
AA1=K1coeff//ChangeContext[#,"Teukolsky`PN`Private`"]&;
AA2=K2coeff//ChangeContext[#,"Teukolsky`PN`Private`"]&;
...
EchoTiming[aux=(norm coeff DoABunchOfStuff[(K1coeff/K1+K2coeff/K2)])//DoABunchOfStuff//DoABunchOfStuff//ReflectGammas,"Assemble"];
aux
]*)


CAmplitudeFreq["Ref",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{aux,aux2,coeff,repls,ReflectGammas,trigPattern,GenSeriesStuff,ExpandTrig,DoABunchOfStuff,\[CurlyEpsilon],\[Kappa],\[CurlyEpsilon]p,\[Tau],K1,K1coeff,K2coeff,K2,A,norm,normOp,nMin,nMax,GammaStuff},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[Kappa]=Sqrt[1-a^2];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
...
normOp=OptionValue["Normalization"];
If[!MemberQ[PossibleNormalizations,normOp],Message[TeukolskyRadialFunctionPN::norm,OptionValue["Normalization"]];normOp="Default"];
DoABunchOfStuff=(#//IgnoreExpansionParameter//SeriesTake[#,order\[CurlyEpsilon]]&//ExpandTrig//GenSeriesStuff)&;
trigPattern=Sin|Cos|Tan|Csc|Sec|Cot|Sinh|Cosh|Tanh|Csch|Sech|Coth;
ExpandTrig=#/. (f:trigPattern)[x_]:>f[Expand[x]]&;
GenSeriesStuff=If[!NumericQ[\[ScriptL]],((#//Expand//SeriesPlusSimplify[#,{\[ScriptL]>=If[NumericQ[\[ScriptS]],Abs[\[ScriptS]],0]}]&)&),Identity];
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,Max[order\[CurlyEpsilon]+2,2]];
...
K1coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0},(I^(-\[Nu]MST+\[ScriptS]) \!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(nMax\)]\(K1coeff\  aMST[n]\)\))/.repls//DoABunchOfStuff];
...
K2coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0},(I^(\[Nu]MST+1+\[ScriptS]) \!\(
...
aux=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=0},norm coeff DoABunchOfStuff[Expand[(K1coeff/K1+K2coeff/K2)]]//DoABunchOfStuff];
aux
]
...
\[ScriptCapitalK]AmplitudeFreq["\[Nu]",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{\[ScriptR]=0,ret,\[CapitalGamma]=Gamma,PH=Pochhammer,SeriesStuff,norm,GammaStuff,order,coeff,sumUpPHCoeff,sumUp,sumDown,\[CurlyEpsilon]p,\[Tau],\[CurlyEpsilon],\[Kappa]=Sqrt[1-a^2],nMax,nMin,jump,jumpCount,repls,repls\[Nu]},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
...
coeff=(E^(I \[CurlyEpsilon] \[Kappa]) I^(\[ScriptS]-\[Nu]MST) (\[CurlyEpsilon] \[Kappa])^(\[ScriptS]-\[Nu]MST-\[ScriptR]) 2^-\[Nu]MST I^-\[ScriptR] (\[CapitalGamma][1-\[ScriptS]-2 I \[CurlyEpsilon]p]))/(\[CapitalGamma][\[ScriptR]+\[Nu]MST+1-\[ScriptS]+I \[CurlyEpsilon]]\[CapitalGamma][1+\[Nu]MST-I \[Tau]]);
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//SeriesStuff];
nMax=order-1;
nMin=-(order+1);
...
(*sumUpPHCoeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},Gamma[1+\[ScriptR]+2 \[Nu]MST]/(Gamma[1-\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+\[Nu]MST-I \[Tau]])/.repls//SeriesStuff];
...
SeriesStuff=(#//IgnoreExpansionParameter//SeriesTake[#,order\[CurlyEpsilon]]&)&;
GammaStuff=(#//ExpandGamma[#,\[ScriptL]+1]&//ExpandPolyGamma[#,\[ScriptL]+1]&)&;
If[!NumericQ[\[ScriptL]],GammaStuff=Identity];
jump[1]=\[ScriptS]+\[ScriptL]+1+n<=0;(*This is the upper Pochhammer in sumDown*)
jumpCount=1+(jump[1]//Boole//Total);
If[!NumericQ[\[ScriptL]],jumpCount=1];
order=order\[CurlyEpsilon]+1//Max[#,3]&;
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order];
repls\[Nu][n_]:=<|\[Nu]MST->(repls[\[Nu]MST]//SeriesTake[#,SeriesLength[repls[aMST[n]]]+jumpCount]&)|>;
coeff=(E^(I \[CurlyEpsilon] \[Kappa]) I^(\[ScriptS]-\[Nu]MST) (\[CurlyEpsilon] \[Kappa])^(\[ScriptS]-\[Nu]MST-\[ScriptR]) 2^-\[Nu]MST I^-\[ScriptR] (\[CapitalGamma][1-\[ScriptS]-2 I \[CurlyEpsilon]p]))/\[CapitalGamma][\[ScriptR]+\[Nu]MST+1-\[ScriptS]+I \[CurlyEpsilon]];
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//SeriesStuff];
nMax=order-1;
nMin=-(order+1);
sumUp=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = \[ScriptR]\), \(nMax\)]\(\((
\*FractionBox[\(
\*SuperscriptBox[\((\(-1\))\), \(n\)]\ PH[1 + \[ScriptS] + I\ \[CurlyEpsilon] + \[Nu]MST, n]\ \[CapitalGamma][1 + \[ScriptR] + 2\ \[Nu]MST + n]\ PH[1 + \[Nu]MST + I\ \[Tau], n]\), \(\(\((n - \[ScriptR])\)!\)\ \[CapitalGamma][1 - \[ScriptS] - I\ \[CurlyEpsilon] + \[Nu]MST + n]\ \[CapitalGamma][1 + \[Nu]MST - I\ \[Tau] + n]\)] /. repls\[Nu][n])\)\((aMST[n]\  /. repls)\)\)\)//SeriesStuff];
(*sumUpPHCoeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},Gamma[1+\[ScriptR]+2 \[Nu]MST]/(Gamma[1-\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+\[Nu]MST-I \[Tau]])/.repls//SeriesStuff];
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},(coeff sumUpPHCoeff)];
*)
sumDown=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
...
\[ScriptCapitalK]AmplitudeFreq["-\[Nu]-1",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{\[ScriptR]=0,ret,\[CapitalGamma]=Gamma,PH=Pochhammer,SeriesStuff,ReflectGammas,norm,flipcoeff,GammaStuff,order,coeff,sumUpPHCoeff,sumUp,sumDown,\[CurlyEpsilon]p,\[Tau],\[CurlyEpsilon],\[Kappa]=Sqrt[1-a^2],nMax,nMin,jump,jumpCount,repls,repls\[Nu]},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
ReflectGammas=#/.{Gamma[x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>a>0,1>\[Kappa]>0,\[ScriptM]>0}]]==-1):>(\[Pi] Csc[\[Pi] x])/Gamma[1-x],PolyGamma[n_,x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>\[Kappa]>0,1>a>0,\[ScriptM]>0}]]==-1):>-\[Pi]^(n+1) Derivative[n][Cot][\[Pi] x]+(-1)^n PolyGamma[n,1-x]}&;
...
If[!NumericQ[\[ScriptL]],jumpCount=1];
order=order\[CurlyEpsilon]+1//Max[#,3]&;
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order+1];
repls\[Nu][n_]:=<|\[Nu]MST->(repls[\[Nu]MST]//SeriesTake[#,SeriesLength[repls[aMST[n]]]+jumpCount]&)|>;
coeff=-(((-1)^(\[ScriptR]-2 \[ScriptS]) I^(1-\[ScriptR]+\[ScriptS]+\[Nu]MST) 2^\[Nu]MST E^(I \[CurlyEpsilon] \[Kappa]) (\[CurlyEpsilon] \[Kappa])^(1-\[ScriptR]+\[ScriptS]+\[Nu]MST) (Cos[2 \[Pi] \[Nu]MST]-Cosh[2 \[Pi] \[CurlyEpsilon]]) Csc[2 \[Pi] \[Nu]MST]^2(* Sin[\[Pi] (\[Nu]MST+I \[Tau])] \[CapitalGamma][1+\[Nu]MST+\[ImaginaryI] \[Tau]]*)(*(-(\[Pi]/Gamma[-\[Nu]MST-\[ImaginaryI] \[Tau]]))*)\[CapitalGamma][1-\[ScriptR]+\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST])/\[Pi]);
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//SeriesStuff//GammaStuff//ExpandTrig];
If[OptionValue["Simplify"],coeff=coeff//SeriesCollect[#,{Gamma[__],PolyGamma[__],Log[__]},(Simplify[#,{1>a>=0,\[ScriptM]\[Element]Integers,\[ScriptL]\[Element]Integers>=0,\[ScriptS]\[Element]Integers}]&)]&];
flipcoeff=If[!NumericQ[\[ScriptL]]||!NumericQ[\[ScriptS]],Sin[\[Pi] (\[Nu]MST+I \[Tau])]  \[CapitalGamma][1-\[ScriptS]-I (\[CurlyEpsilon]+\[Tau])]\[CapitalGamma][1+\[Nu]MST+I \[Tau]],(-((\[Pi] \[CapitalGamma][1-\[ScriptS]-I (\[CurlyEpsilon]+\[Tau])])/Gamma[-\[Nu]MST-I \[Tau]]))];
flipcoeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},flipcoeff/.repls//SeriesStuff//GammaStuff//ReflectGammas//GammaStuff//ExpandTrig//ExpandTrig];
If[OptionValue["Simplify"],flipcoeff=flipcoeff//ExpandGamma//ExpandPolyGamma//SeriesCollect[#,{Gamma[__],PolyGamma[__]},(Simplify[#,{1>a>=0,\[ScriptM]\[Element]Integers,\[ScriptL]\[Element]Integers>=0,\[ScriptS]\[Element]Integers}]&)]&//GammaStuff];
nMax=order-1;
nMin=-(order+1);
sumUp=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(-\[ScriptR]\)]\(\((
...
If[OptionValue["Simplify"],sumUp=sumUp//SeriesCollect[#,{\[Omega],EulerGamma,\[Pi]},(Simplify[#,{1>a>=0,\[ScriptM]\[Element]Integers,\[ScriptL]\[Element]Integers>=0,\[ScriptS]\[Element]Integers}]&)]&];
(*sumUpPHCoeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},Gamma[1+\[ScriptR]+2 \[Nu]MST]/(Gamma[1-\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] Gamma[1+\[Nu]MST-I \[Tau]])/.repls//SeriesStuff];
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},(coeff sumUpPHCoeff)];
*)
sumDown=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
...
(*\[ScriptCapitalK]AmplitudeFreq["-\[Nu]-1",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{\[ScriptR]=0,ret,\[CapitalGamma]=Gamma,PH=Pochhammer,ReflectGammas,order,coeff,norm,DoABunchOfStuff,SeriesStuff,GammaStuff,sumUpPHCoeff,sumUp,sumDown,\[CurlyEpsilon]p,\[Tau],\[CurlyEpsilon],\[Kappa],nMax,nMin,jump,jumpCount,repls,repls\[Nu]},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
SeriesStuff=(#//IgnoreExpansionParameter//SeriesTake[#,order\[CurlyEpsilon]]&//ExpandTrig)&;
ReflectGammas=#/.{Gamma[x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>a>0,1>\[Kappa]>0,\[ScriptM]>0}]]==-1):>(\[Pi] Csc[\[Pi] x])/Gamma[1-x],PolyGamma[n_,x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>\[Kappa]>0,1>a>0,\[ScriptM]>0}]]==-1):>-\[Pi]^(n+1) Derivative[n][Cot][\[Pi] x]+(-1)^n PolyGamma[n,1-x]}&;
GammaStuff=(#//ExpandPolyGamma[#,\[ScriptL]]&//ExpandGamma[#,\[ScriptL]]&//ReflectGammas//ExpandPolyGamma[#,-\[ScriptL]]&//ExpandGamma[#,-\[ScriptL]]&)&;
If[!NumericQ[\[ScriptL]],GammaStuff=((#//ReflectGammas//ExpandGamma//ExpandPolyGamma)&)];
...
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order];
repls\[Nu][n_]:=<|\[Nu]MST->(repls[\[Nu]MST]//SeriesTake[#,SeriesLength[repls[aMST[n]]]+jumpCount]&)|>;
coeff=(I^(1-\[ScriptR]+\[ScriptS]+\[Nu]MST) 2^(1+\[Nu]MST) E^(I \[CurlyEpsilon] \[Kappa]) (\[CurlyEpsilon] \[Kappa])^(1-\[ScriptR]+\[ScriptS]+\[Nu]MST) Csc[\[Pi] (\[ScriptR]-2 \[Nu]MST)] Sin[\[Pi] (\[ScriptR]-\[ScriptS]+I \[CurlyEpsilon]-\[Nu]MST)] \[CapitalGamma][1-\[ScriptS]-2 I \[CurlyEpsilon]p] \[CapitalGamma][1-\[ScriptR]+\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST])/\[CapitalGamma][1-\[ScriptR]+2 \[Nu]MST];
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},SeriesStuff[coeff/. repls]];
nMax=order-1;
nMin=-(order+1);
sumUp=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(-\[ScriptR]\)]\(\((
...
\[ScriptCapitalK]AmplitudeFreq["Ratio",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{\[ScriptR]=0,ret,\[CapitalGamma]=Gamma,PH=Pochhammer,SeriesStuff,ReflectGammas,norm,GammaStuff,order,flipcoeff,coeff,sumUpPHCoeff,sumUp1,sumUp2,sumDown1,sumDown2,\[CurlyEpsilon]p,\[Tau],\[CurlyEpsilon],\[Kappa]=Sqrt[1-a^2],nMax,nMin,jump,jumpCount,repls,repls\[Nu]},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
ReflectGammas=#/.{Gamma[x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>a>0,1>\[Kappa]>0,\[ScriptM]>0}]]==-1):>(\[Pi] Csc[\[Pi] x])/Gamma[1-x],PolyGamma[n_,x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>\[Kappa]>0,1>a>0,\[ScriptM]>0}]]==-1):>-\[Pi]^(n+1) Derivative[n][Cot][\[Pi] x]+(-1)^n PolyGamma[n,1-x]}&;
...
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order+1];
repls\[Nu][n_]:=<|\[Nu]MST->(repls[\[Nu]MST]//SeriesTake[#,SeriesLength[repls[aMST[n]]]+jumpCount]&)|>;
coeff=-((I (-1)^(\[ScriptR]-2 \[ScriptS]) (2 I)^(2 \[Nu]MST) \[CurlyEpsilon] \[Kappa] (\[CurlyEpsilon] \[Kappa])^(2 \[Nu]MST) (Cos[2 \[Pi] \[Nu]MST]-Cosh[2 \[Pi] \[CurlyEpsilon]]) Csc[2 \[Pi] \[Nu]MST]^2 \[CapitalGamma][1-\[ScriptR]+\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] \[CapitalGamma][1+\[ScriptR]-\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST])/\[Pi]);
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//SeriesStuff//ReflectGammas//ExpandTrig];
If[OptionValue["Simplify"],coeff=coeff//SeriesCollect[#,{Gamma[__],PolyGamma[__],Log[__]},(Simplify[#,{1>a>=0,\[ScriptM]\[Element]Integers,\[ScriptL]\[Element]Integers>=0,\[ScriptS]\[Element]Integers}]&)]&];
flipcoeff=If[!NumericQ[\[ScriptL]]||!NumericQ[\[ScriptS]],Sin[\[Pi] (\[Nu]MST+I \[Tau])] \[CapitalGamma][1+\[Nu]MST+I \[Tau]]\[CapitalGamma][1+\[Nu]MST-I \[Tau]],(-((\[Pi] \[CapitalGamma][1+\[Nu]MST-I \[Tau]])/\[CapitalGamma][-\[Nu]MST-I \[Tau]]))];
flipcoeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},flipcoeff/.repls//SeriesStuff//GammaStuff//ReflectGammas//GammaStuff//ExpandTrig//ExpandTrig];
If[OptionValue["Simplify"],flipcoeff=flipcoeff//ExpandGamma//ExpandPolyGamma//SeriesCollect[#,{Gamma[__],PolyGamma[__]},(Simplify[#,{1>a>=0,\[ScriptM]\[Element]Integers,\[ScriptL]\[Element]Integers>=0,\[ScriptS]\[Element]Integers}]&)]&//GammaStuff];
nMax=order-1;
nMin=-(order+1);
...
(*\[ScriptCapitalK]AmplitudeFreq["Ratio",OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[CurlyEpsilon]_]:=Module[{\[ScriptR]=0,ret,\[CapitalGamma]=Gamma,PH=Pochhammer,SeriesStuff,ReflectGammas,norm,GammaStuff,order,coeff,sumUpPHCoeff,sumUp1,sumUp2,sumDown1,sumDown2,\[CurlyEpsilon]p,\[Tau],\[CurlyEpsilon],\[Kappa]=Sqrt[1-a^2],nMax,nMin,jump,jumpCount,repls,repls\[Nu]},
\[CurlyEpsilon]=2 \[Omega] \[Gamma];
\[CurlyEpsilon]p=(\[CurlyEpsilon]+\[Tau])/2;
\[Tau]=(-a \[ScriptM]+\[CurlyEpsilon])/\[Kappa];
ReflectGammas=#/.{Gamma[x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>a>0,1>\[Kappa]>0,\[ScriptM]>0}]]==-1):>(\[Pi] Csc[\[Pi] x])/Gamma[1-x],PolyGamma[n_,x_]/;(Quiet[Simplify[-I Sign[Select[x//If[NumericQ[\[ScriptS]],Identity,ReplaceAll[\[ScriptS]->1]]//If[NumericQ[\[ScriptL]],Identity,ReplaceAll[\[ScriptL]->1]],(!IntegerQ[#]&)]],{1>\[Kappa]>0,1>a>0,\[ScriptM]>0}]]==-1):>-\[Pi]^(n+1) Derivative[n][Cot][\[Pi] x]+(-1)^n PolyGamma[n,1-x]}&;
...
repls=MSTCoefficientsInternalFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order+1];
repls\[Nu][n_]:=<|\[Nu]MST->(repls[\[Nu]MST]//SeriesTake[#,SeriesLength[repls[aMST[n]]]+jumpCount]&)|>;
coeff=-((I (-1)^(\[ScriptR]-2 \[ScriptS]) (2 I)^(2 \[Nu]MST) \[CurlyEpsilon] \[Kappa] (\[CurlyEpsilon] \[Kappa])^(2 \[Nu]MST) (Cos[2 \[Pi] \[Nu]MST]-Cosh[2 \[Pi] \[CurlyEpsilon]]) Csc[2 \[Pi] \[Nu]MST]^2 Sin[\[Pi] (\[Nu]MST+I \[Tau])] \[CapitalGamma][1-\[ScriptR]+\[ScriptS]-I \[CurlyEpsilon]+\[Nu]MST] \[CapitalGamma][1+\[ScriptR]-\[ScriptS]+I \[CurlyEpsilon]+\[Nu]MST])/\[Pi]);
coeff=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},coeff/.repls//SeriesStuff//ReflectGammas//ExpandTrig];
nMax=order-1;
nMin=-(order+1);
sumUp2=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = nMin\), \(-\[ScriptR]\)]\(\((
\*FractionBox[\(\(\ \)\(PH[1 + n - \[ScriptS] - I\ \[CurlyEpsilon] + \[Nu]MST, \(-n\)]\ PH[1 + n + \[Nu]MST - I\ \[Tau], \(-n\)]\ \ \[CapitalGamma][1 + n + \[ScriptS] + I\ \[CurlyEpsilon] + \[Nu]MST]\ \[CapitalGamma][1 + n + \[Nu]MST + I\ \[Tau]]\)\), \(\(\((\(-n\) - \[ScriptR])\)!\)\ \[CapitalGamma][2 + n - \[ScriptR] + 2\ \[Nu]MST]\)] /. repls\[Nu][n])\)\((aMST[n]\  /. repls)\)\)\)//SeriesStuff//ReflectGammas//ExpandTrig];
sumDown2=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = \(-\[ScriptR]\)\), \(nMax\)]\(\((
\*FractionBox[\(PH[1 + n - \[ScriptS] + I\ \[CurlyEpsilon] + \[Nu]MST, \(-n\)]\ \ \[CapitalGamma][1 + n - \[ScriptR] + 2\ \[Nu]MST]\), \(\(\ \)\(\(\((n + \[ScriptR])\)!\)\ PH[1 + n + \[ScriptS] - I\ \[CurlyEpsilon] + \[Nu]MST, \(-n\)]\)\)] /. repls\[Nu][n])\)\((aMST[n] /. repls)\)\)\)//SeriesStuff//ReflectGammas//ExpandTrig];
sumDown1=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=0},\!\(
\*UnderoverscriptBox[\(\[Sum]\), \(n = \[ScriptR]\), \(nMax\)]\(\((
\*FractionBox[\(
\*SuperscriptBox[\((\(-1\))\), \(n\)]\ PH[1 + \[ScriptS] + I\ \[CurlyEpsilon] + \[Nu]MST, n]\ \[CapitalGamma][1 + \[ScriptR] + 2\ \[Nu]MST + n]\ PH[1 + \[Nu]MST + I\ \[Tau], n]\), \(\(\((n - \[ScriptR])\)!\)\ \[CapitalGamma][1 - \[ScriptS] - I\ \[CurlyEpsilon] + \[Nu]MST + n]\ \[CapitalGamma][1 + \[Nu]MST - I\ \[Tau] + n]\)] /. repls\[Nu][n])\)\((aMST[n]\  /. repls)\)\)\)//SeriesStuff];
...
norm=If[OptionValue["Normalization"]==="SasakiTagoshi",ISymmetryFactorFreq["Ratio"][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon]],1];
ret=norm coeff sumUp1 sumUp2/(sumDown1 sumDown2) //SeriesStuff//GammaStuff;
ret
]*)
...
order\[CurlyEpsilon]=Ceiling[order\[Eta],3]/3;
order=If[OptionValue["FreqRep"],order\[Eta],order\[CurlyEpsilon]];
aux=\[ScriptCapitalK]AmplitudeFreq[sol,"Normalization"->OptionValue["Normalization"],"Simplify"->OptionValue["Simplify"]][\[ScriptS],\[ScriptL],\[ScriptM],a,order];
...
If[!NumericQ[\[ScriptL]],aux=aux//TrigToExp//ReplaceAll[E^aa_:>Simplify[E^Expand[aa],{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptM]\[Element]Integers,\[ScriptL]>=Abs[\[ScriptS]]}]]//Expand//SeriesPlusSimplify[#,\[ScriptL]>=Abs[\[ScriptS]]]&];
(*If[OptionValue["Simplify"],aux=aux//SeriesExpand//SeriesCollect[#,{E^__,Log[__],Gamma[__],PolyGamma[__]},(Simplify[#,{1>a>=0,\[ScriptM]\[Element]Integers,\[ScriptL]\[Element]Integers>=0,\[ScriptS]\[Element]Integers}]&)]&];*)
aux
]
...
(* ::Text:: *)
(*This factor is here such that both \!\(\*SubsuperscriptBox[\(R\), \(C\), \(\[Nu]\)]\) and Subscript[K, \[Nu]] obey the symmry SuperStar[(Subscript[X, \[ScriptL] m \[Omega]])]==Subscript[X, \[ScriptL] -m -\[Omega]]. In Sasaki Tagoshi this is onyl true for their combination Subscript[K, \[Nu]] \!\(\*SubsuperscriptBox[\(R\), \(C\), \(\[Nu]\)]\) *)
...
(* ::Text:: *)
(*This factor is here to remove logarithms of omega from Subscript[R, In]. The corresponding normalization was used in arxiv:2607.12151 *)
...
phaseShiftFreq[s_,\[ScriptL]_,m_,a_,order_,P_,opt:OptionsPattern[]]:=Module[{aux,CTS,\[CurlyEpsilon]b,Binc,Bref},
CTS=CTeukolskyStarobinsky[s,\[ScriptL],m,a,\[Omega],{\[Gamma],order},P];
...
If[!NumericQ[\[ScriptL]],aux=aux//TrigToExp//ReplaceAll[E^aa_:>Simplify[E^Expand[aa],{\[ScriptL]\[Element]Integers,\[ScriptL]>=Abs[s]}]]];
aux
]
...
phaseShift[-2,\[ScriptL]_,\[ScriptM]_,a_,order\[Eta]_,OptionsPattern[]]:=Module[{aux1,aux2,order\[CurlyEpsilon],\[ScriptS]=-2,ret},
If[OptionValue["FreqRep"],
aux1=phaseShiftFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order\[Eta],1,"Normalization"->OptionValue["Normalization"]];
,
(*else*)
order\[CurlyEpsilon]=Ceiling[order\[Eta],3]/3;
aux1=phaseShiftFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon],1,"Normalization"->OptionValue["Normalization"]];
aux1=aux1//ChangeSeriesParameter[#,\[Eta]^3]&//SeriesTake[#,order\[Eta]]&;
aux1
];
If[OptionValue["Simplify"],aux1=aux1//ExpandGamma//ExpandPolyGamma//SeriesCollect[#,{Gamma[__],Log[__],PolyGamma[__]},(Simplify[#,{\[ScriptL]\[Element]Integers,\[ScriptL]>=2,\[ScriptM]\[Element]Integers,1>a>=0}]&)]&];
If[OptionValue["FreqRep"],
aux2=phaseShiftFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order\[Eta],-1,"Normalization"->OptionValue["Normalization"]];
,
(*else*)
order\[CurlyEpsilon]=Ceiling[order\[Eta],3]/3;
aux2=phaseShiftFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order\[CurlyEpsilon],-1,"Normalization"->OptionValue["Normalization"]];
aux2=aux2//ChangeSeriesParameter[#,\[Eta]^3]&//SeriesTake[#,order\[Eta]]&;
aux2
];
If[OptionValue["Simplify"],aux2=aux2//ExpandGamma//ExpandPolyGamma//SeriesCollect[#,{Gamma[__],Log[__],PolyGamma[__]},(Simplify[#,{\[ScriptL]\[Element]Integers,\[ScriptL]>=2,\[ScriptM]\[Element]Integers,1>a>=0}]&)]&];
ret=<|"+"->aux1,"-"->aux2|>;
ret
]


phaseShift[\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,order\[Eta]_,OptionsPattern[]]/;(\[ScriptS]===-1||\[ScriptS]===0):=Module[{aux1,aux2,order\[CurlyEpsilon],ret},
If[OptionValue["FreqRep"],
aux1=phaseShiftFreq[\[ScriptS],\[ScriptL],\[ScriptM],a,order\[Eta],1,"Normalization"->OptionValue["Normalization"]];
,
(*else*)
...
TeukolskyAmplitudePN["K\[Nu]",opt:OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,\[Omega]Var_,{\[Eta]Var_,order\[Eta]_}] :=Module[{aux},
If[MatchQ[\[ScriptL],_Symbol],Message[MSTCoefficientsPN::warn,Max[order\[Eta]-1,2],\[ScriptL]]];
aux=\[ScriptCapitalK]Amplitude["\[Nu]",opt][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[Eta]];
aux=aux/.{\[Omega]->\[Omega]Var,\[Gamma]->\[Eta]Var,\[Eta]->\[Eta]Var};
aux
]
...
TeukolskyAmplitudePN["K",opt:OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,a_,\[Omega]Var_,{\[Eta]Var_,order\[Eta]_}] :=Module[{aux},
If[MatchQ[\[ScriptL],_Symbol],Message[MSTCoefficientsPN::warn,Max[order\[Eta]-1,2],\[ScriptL]]];
aux=\[ScriptCapitalK]Amplitude["Ratio",opt][\[ScriptS],\[ScriptL],\[ScriptM],a,order\[Eta]];
aux=aux/.{\[Omega]->\[Omega]Var,\[Gamma]->\[Eta]Var,\[Eta]->\[Eta]Var};
aux
]
...
C1=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=Abs[\[ScriptS]],\[ScriptM]\[Element]Integers},PNScalingsInternal[coeffUp I^(\[Nu]MST+1+\[ScriptS]) ]/.repls];
C2=Assuming[{\[ScriptS]\[Element]Integers,\[ScriptL]\[Element]Integers,\[ScriptL]>=Abs[\[ScriptS]],\[ScriptM]\[Element]Integers},PNScalingsInternal[coeffUp I^(-\[Nu]MST+\[ScriptS]) ((I E^(-I \[Pi] \[Nu]MST) Sin[\[Pi] (\[Nu]MST-\[ScriptS]+I \[CurlyEpsilon])])/Sin[\[Pi] (\[Nu]MST+\[ScriptS]-I \[CurlyEpsilon])])]/.repls];
If[NumericQ[\[ScriptL]],
	aux=C1 RC2+C2 RC1;,
	(*else*)
...
TeukolskyRadialFunctionPN[s_, l_, m_, a_, r0_,{varPN_,order_},assoc_][r_] :=assoc["RadialFunction"][r]
...
Derivative[n_Integer][trf_TeukolskyRadialFunctionPN][r_]:=(*trf[[6,1]]^(2 n)*) Derivative[n][trf[[-1]]["RadialFunction"]][r]
...
RadialSourcedAssociation["CO",opt:OptionsPattern[]][\[ScriptS]_,\[ScriptL]_,\[ScriptM]_,aVar_,r0Var_,{varPN_,order_}]:=Assuming[{varPN>0,r0>0,r>0,1>a>=0,\[ScriptA]>=0},Module[{aux,r,r0,\[Omega]FourierScaled,options,\[CapitalOmega]\[Phi]\[Gamma],\[Omega]\[Gamma],sourceRepls,m,\[CapitalOmega]\[Phi],\[CapitalOmega]\[Phi]Scaled,length,arg,args,ret,\[Omega]Fourier,orbit,Btrans,Ctrans,Scoeffs,SCoeffsF,Rin,dRin,ddRin,Rup,dRup,ddRup,wronskian,source,sourceF,sourceCoeffs,minOrder,cUp,cIn,cUpU,cInU,deltaCoeff,deltaCoeffF,innerF,outerF,inner,outer,radialF,radial,ampAssoc},
...
\[CapitalOmega]\[Phi]\[Gamma]=If[OptionValue["InactiveHarmonics"],Inactive[KerrGeoFrequencies][aVar,r0Var,0,1]["\!\(\*SubscriptBox[\(\[CapitalOmega]\), \(\[Phi]\)]\)"],KerrGeoFrequencies[\[Gamma] aVar,r0Var,0,1]["\!\(\*SubscriptBox[\(\[CapitalOmega]\), \(\[Phi]\)]\)"]];
\[Omega]\[Gamma]=If[\[ScriptM]===0,Style[0,Orange]\[CapitalOmega]\[Phi]\[Gamma],\[ScriptM] \[CapitalOmega]\[Phi]\[Gamma]];
...
If[Length[args]!=1,Abort[]];
arg=#&@@args;
If[!OptionValue["\[ScriptCapitalI]Only"],cIn=(-1)^(\[ScriptS]+1) (Kerr\[CapitalDelta][aVar,varPN^-2  r]^\[ScriptS] source Rup[r])/wronskian,cIn=Missing["Not Computed"]];
...
deltaCoeff=radial/.HeavisideTheta->(0&);
deltaCoeffF=deltaCoeff/.r->#&;
(*Scoeffs=SeriesToSCoeffs[radial];*)
minOrder=radial//SeriesMinOrder;
radialF=radial/.r->#&;
innerF=inner/.r->#&;
outerF=outer/.r->#&;
sourceF=source[r]/.sourceRepls/.r->#&;
(*SCoeffsF=Scoeffs/.r->#&;*)
orbit=KerrGeoOrbit[aVar,r0Var,0,1];
options=Options[RadialSourcedAssociation][[All,1]];
options=Table[opVal->OptionValue[opVal],{opVal,options}]//Association;
ret=<|"s"->\[ScriptS],"l"->\[ScriptL],"m"->\[ScriptM],"a"->aVar,"r0"->r0Var,"PN"->{varPN,order},"RadialFunction"->radialF(*,"CoefficientList"->SCoeffsF*),("ExtendedHomogeneous"->"\[ScriptCapitalI]")->outerF,("ExtendedHomogeneous"->"\[ScriptCapitalH]")->innerF,"\[Delta]"->deltaCoeffF,"Amplitudes"->ampAssoc,"Wronskian"->wronskian,"Source"->sourceF,"SeriesMinOrder"->minOrder,"RadialFunctions"->aux,"\[Omega]"->Simplify[Activate[\[Omega]Fourier/.varPN->1],r0Var>0],"Orbit"->orbit,"Options"->options|>;
ret
]
]
...
Options[TeukolskyPointParticleModePN]={"Normalization"->"Default","Simplify"->True,"InactiveHarmonics"->False,"\[ScriptCapitalI]Only"->False}
