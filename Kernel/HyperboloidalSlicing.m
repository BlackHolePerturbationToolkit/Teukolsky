...
psi[s_, l_, \[Omega]_, "In", ndsolveopts___][rmax_?NumericQ] := psi[s, l, \[Omega], "In", ndsolveopts][{Automatic, rmax}];
psi[s_, l_, \[Omega]_, "Up", ndsolveopts___][rmin_?NumericQ] := psi[s, l, \[Omega], "Up", ndsolveopts][{rmin, Automatic}];

psi[s_, l_, \[Omega]_, bc_, ndsolveopts___][{rmin_, rmax_}] :=
 Module[{bcFunc, psiBC, dpsidrBC, rBC, rMin, rMax, H, soln},
    bcFunc = Lookup[<|"In" -> TeukolskyInBC, "Up" -> TeukolskyUpBC|>, bc];
    {psiBC, dpsidrBC, rBC} = bcFunc[s, l, \[Omega], Lookup[{ndsolveopts}, WorkingPrecision, Precision[\[Omega]]]];
...
psi[s_, l_, \[Omega]_, bc_, ndsolveopts___][All] :=
 Module[{bcFunc, psiBC, dpsidrBC, rBC, rMin, rMax, H, soln},
    bcFunc = Lookup[<|"In" -> TeukolskyInBC, "Up" -> TeukolskyUpBC|>, bc];
    {psiBC, dpsidrBC, rBC} = bcFunc[s, l, \[Omega], Lookup[{ndsolveopts}, WorkingPrecision, Precision[\[Omega]]]];
    soln = r^-(2s+1) f[r]^-s Exp[I \[Omega] H rs[r]]AllIntegrator[s, l, \[Omega], psiBC, dpsidrBC, rBC, H, ndsolveopts]
];

psi[s_, l_, \[Omega]_, bc_, ndsolveopts___][None] := $Failed;


Integrator[s_,l_,\[Omega]_,y1BC_,y2BC_,rBC_,rmin_?NumericQ,rmax_?NumericQ,H_?NumericQ,ndsolveopts___]:=Module[{y1,y2,r},
	Quiet[NDSolveValue[
...
AllIntegrator[s_,l_,\[Omega]_,y1BC_,y2BC_,rBC_,potential_,H_?NumericQ,ndsolveopts___][rval:(_?NumericQ | {_?NumericQ..})] := Module[{y1,y2,r},
	Quiet[NDSolveValue[
		{y1'[r]==y2[r],(-(((1-2/r) (l (1+l)+(2 (1+s))/r-s (1+s)))/r^2)+(2 I (-1-H+(1-H) (1-2/r) r) s \[Omega])/r^2+(1-H^2) \[Omega]^2) y1[r]+(1-2/r) (2/r^2-(2 (-1+r) s)/r^2+2 I H \[Omega]) y2[r]+(1-2/r)^2 Derivative[1][y2][r]==0,y1[rBC]==y1BC,y2[rBC]==y2BC},
		y1[rval],
...
Derivative[n_][AllIntegrator[s_,l_,\[Omega]_,y1BC_,y2BC_,rBC_,potential_,H_?NumericQ,ndsolveopts___]][rval:(_?NumericQ | {_?NumericQ..})] := Module[{y1,y2,r},
	Quiet[NDSolveValue[
		{y1'[r]==y2[r],(-(((1-2/r) (l (1+l)+(2 (1+s))/r-s (1+s)))/r^2)+(2 I (-1-H+(1-H) (1-2/r) r) s \[Omega])/r^2+(1-H^2) \[Omega]^2) y1[r]+(1-2/r) (2/r^2-(2 (-1+r) s)/r^2+2 I H \[Omega]) y2[r]+(1-2/r)^2 Derivative[1][y2][r]==0,y1[rBC]==y1BC,y2[rBC]==y2BC},
