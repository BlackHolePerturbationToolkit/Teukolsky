...
Options[H2F1Exact]={RW-> False};
H2F1Exact[n_, s_, \[Nu]_, \[Tau]_, \[Epsilon]_, x_,OptionsPattern[]] :=Module[{a,b,c}, If[OptionValue[RW],a=\[Nu]+s+1-I \[Epsilon];
b=-\[Nu]+s-I \[Epsilon];
c=1-2 I \[Epsilon];,
...
Options[H2F1Up]={RW-> False};
H2F1Up[n_,s_,\[Nu]_,\[Tau]_,\[Epsilon]_,x_,OptionsPattern[]]:=Module[{a,b,c},If[OptionValue[RW],a=\[Nu]+s+1-I \[Epsilon];
b=-\[Nu]+s-I \[Epsilon];c=1-2 I \[Epsilon];,
a=\[Nu]+1-I \[Tau]; b=-\[Nu]-I \[Tau]; c=1-s-I(\[Epsilon]+\[Tau]);];
...
Options[H2F1Down]={RW-> False};
H2F1Down[n_,s_,\[Nu]_,\[Tau]_,\[Epsilon]_,x_,OptionsPattern[]]:=Module[{a,b,c},If[OptionValue[RW],a=\[Nu]+s+1-I \[Epsilon];
b=-\[Nu]+s-I \[Epsilon];c=1-2 I \[Epsilon];,
a=\[Nu]+1-I \[Tau]; b=-\[Nu]-I \[Tau]; c=1-s-I(\[Epsilon]+\[Tau]);];
...
Options[dH2F1Exact]={RW-> False};
dH2F1Exact[n_, s_, \[Nu]_, \[Tau]_, \[Epsilon]_, x_,OptionsPattern[]] := Module[{a,b,c},If[OptionValue[RW],a=\[Nu]+s+1-I \[Epsilon];
b=-\[Nu]+s-I \[Epsilon];c=1-2 I \[Epsilon];,
a=\[Nu]+1-I \[Tau]; b=-\[Nu]-I \[Tau]; c=1-s-I(\[Epsilon]+\[Tau]);];
...
*)
Options[dH2F1Up]={RW-> False};
dH2F1Up[n_,s_,\[Nu]_,\[Tau]_,\[Epsilon]_,x_,OptionsPattern[]]:=Module[{a,b,c},If[OptionValue[RW],a=\[Nu]+s+1-I \[Epsilon];
b=-\[Nu]+s-I \[Epsilon];c=1-2 I \[Epsilon];,
a=\[Nu]+1-I \[Tau]; b=-\[Nu]-I \[Tau]; c=1-s-I(\[Epsilon]+\[Tau]);];
...
Options[dH2F1Down]={RW-> False};
dH2F1Down[n_,s_,\[Nu]_,\[Tau]_,\[Epsilon]_,x_,OptionsPattern[]]:=Module[{a,b,c},If[OptionValue[RW],a=\[Nu]+s+1-I \[Epsilon];
b=-\[Nu]+s-I \[Epsilon];c=1-2 I \[Epsilon];,
a=\[Nu]+1-I \[Tau]; b=-\[Nu]-I \[Tau]; c=1-s-I(\[Epsilon]+\[Tau]);];
...
(*Check if we are using ans or anT's. Code so far looks like converting anT to anRW*)
Options[f]={RW-> False};
f[q_, \[Epsilon]_, \[Kappa]_, \[Tau]_, \[Nu]_, \[Lambda]_, s_, m_, nf_, OptionsPattern[]] :=
  If[OptionValue[RW],Pochhammer[-\[Nu]+s-I \[Epsilon],-nf]Pochhammer[\[Nu]+1+s-I \[Epsilon],nf](-1)^nf Pochhammer[\[Nu]+I \[Epsilon]+1,nf]/Pochhammer[\[Nu]-I \[Epsilon]+1,nf],1]fT[q, \[Epsilon], \[Kappa], \[Tau], \[Nu], \[Lambda], s, m, nf];
(* should be:
f[q_, \[Epsilon]_, \[Kappa]_, \[Tau]_, \[Nu]_, \[Lambda]_, s_, m_, nf_, RW_:True] :=
  If[RW,Pochhammer[-\[Nu]+s-I \[Epsilon],-nf]Pochhammer[\[Nu]+1+s-I \[Epsilon],nf]fT[q, \[Epsilon], \[Kappa], \[Tau], \[Nu], \[Lambda], s, m, nf];*)
(******************************************************************************)
...
Options[MSTRadialIn]={RW-> False};
MSTRadialIn[s_Integer, l_Integer, m_Integer, q_, \[Epsilon]_, \[Nu]_, \[Lambda]_, norm_,OptionsPattern[]][r_?InexactNumberQ] := 
 ...
 Internal`InheritedBlock[{\[Alpha], \[Beta], \[Gamma], fT},
  ...
  H2F1[n : (0 | 1)] := H2F1[n] = H2F1Exact[n, s, \[Nu], \[Tau], \[Epsilon], x,OptionValue[RW]];

  H2F1[n_Integer] := H2F1[n] =
   Module[{t1, t2, res},
    {t1, t2} = If[n>0, H2F1Up[n, s, \[Nu], \[Tau], \[Epsilon], x,OptionValue[RW]], H2F1Down[n, s, \[Nu], \[Tau], \[Epsilon], x,OptionValue[RW]]];
    res = t1 + t2;
    If[Max[Abs[{t1, t2}/res]] > 2.,
      res = H2F1Exact[n, s, \[Nu], \[Tau], \[Epsilon], x,OptionValue[RW]];
    ];
    res
  ];
 
  resUp = resDown = 0;
(*need to specify RW=True option? For Teukolsky calc leave this option out for now. Should these be H2F1's or H2F1Exact?*)
  nUp = 0;
  While[resUp != (resUp += 
    f[q, \[Epsilon], \[Kappa], \[Tau], \[Nu], \[Lambda], s, m, nUp,OptionValue[RW]] H2F1[nUp]),
    nUp++;
  ];

  nDown = -1;
  While[resDown != (resDown += 
    f[q, \[Epsilon], \[Kappa], \[Tau], \[Nu], \[Lambda], s, m, nDown,OptionValue[RW]] H2F1[nDown]), 
    nDown--;
  ];
(*pin=resUp+resDown. Note below eqn in from 116 in MST See Adrian v2. Has both versions, as long as \[Kappa] is 1 for RW version.*)
...
Derivative[1][MSTRadialIn[s_Integer, l_Integer, m_Integer, q_, \[Epsilon]_, \[Nu]_, \[Lambda]_, norm_]][r_?InexactNumberQ] :=
 ...
 Internal`InheritedBlock[{\[Alpha], \[Beta], \[Gamma], fT},
  ...
  resUp = resDown = 0;
(*do these derivs need to be fixed too? Assume recurrence relations are correct?*)
  nUp = 0;
  While[resUp != (resUp += 
...
(*in this case we now have pin'= resDown +resUp?*)
  (*\[Epsilon] \[Kappa]? In norm? Also need to fix these - see v2 Adrian. Is product and chain rule implemented correctly here??*)
